...
// ax-pjt-dashboard (홈 대시보드 읽기 전용 소스)
if (!envNonEmpty("DASHBOARD_SUPABASE_URL") && envNonEmpty("AX_PJT_DASHBOARD_SUPABASE_URL")) {
  process.env.DASHBOARD_SUPABASE_URL = process.env.AX_PJT_DASHBOARD_SUPABASE_URL.trim();
}
if (
  !envNonEmpty("DASHBOARD_SUPABASE_SERVICE_ROLE_KEY") &&
  envNonEmpty("DASHBOARD_SUPABASE_SECRET_KEY")
) {
  process.env.DASHBOARD_SUPABASE_SERVICE_ROLE_KEY =
    process.env.DASHBOARD_SUPABASE_SECRET_KEY.trim();
}
if (
  !envNonEmpty("DASHBOARD_SUPABASE_SERVICE_ROLE_KEY") &&
  envNonEmpty("AX_PJT_DASHBOARD_SUPABASE_SECRET_KEY")
) {
  process.env.DASHBOARD_SUPABASE_SERVICE_ROLE_KEY =
    process.env.AX_PJT_DASHBOARD_SUPABASE_SECRET_KEY.trim();
}
...
function isDashboardSupabaseConfigured() {
  return (
    envNonEmpty("DASHBOARD_SUPABASE_URL") &&
    envNonEmpty("DASHBOARD_SUPABASE_SERVICE_ROLE_KEY")
  );
}
